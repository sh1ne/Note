...
1. **Note List Button** - Shows all notes (except 4 staple notes)
2. **Plus Button** - Create new note
3. **Tab 1** - "Scratch" (icon + text)
4. **Tab 2** - "Now" (icon + text)
5. **Tab 3** - "Short-Term" (icon + text)
6. **Tab 4** - "Long-term" (icon + text)
7. **Tab 5** - "More" (icon: "...", text: "More")

### Note List View (Button to left of Plus)
- **Button**: Opens menu with all notes (except 4 "staple tabs" - those are always in bottom nav)
- **Display**: List of notes with title + preview
- **Behavior**: 
  - Click note → temporarily appears in Tab 1 (Scratch) for viewing/editing
  - After closing, note disappears from bottom bar
  - Access again from "all notes" list if needed
  - Note does NOT permanently move to Tab 1
- **Images**: Treated same as other notes (show thumbnail if has images)
- **Date**: Show "Date Updated" in header
...
### Note Creation
- **Plus Button** - Creates new note in current tab
- Located to left of the 5 tabs in bottom nav
...
- Theme toggle (Dark/Light/Purple/Blue)
- Export notes
- Force update to cloud
- Notebook management (create, switch, delete)
